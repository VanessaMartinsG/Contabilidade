import { getAccount, getStatement, deleteTask } from './apiHandler.js';
import { setUserMoneyCards, modalEdit } from './dashboard.js';
import { populateTaskModal } from './modal.js';
...
export async function updateStatements() {
    ...
    if (userStatements != null) {
        ...
        modalEdit();
        eventDeleteTask(userData);
        eventEditTask(userData);
    }
}

function eventDeleteTask(user) {
    const deleteIcons = document.querySelectorAll(".reg__del");
    deleteIcons.forEach(icon => {
        icon.addEventListener('click', async () => {
            const task = { id: icon.getAttribute("id") };
            const response = await deleteTask(user, task);
            updateStatements();
        });
    });
}

function eventEditTask(user) {
    const editIcons = document.querySelectorAll(".reg__edit");
    editIcons.forEach(icon => {
        icon.addEventListener('click', async () => {
            const task = { id: icon.getAttribute("id") };
            populateTaskModal(user, task);
        });
    });
}
